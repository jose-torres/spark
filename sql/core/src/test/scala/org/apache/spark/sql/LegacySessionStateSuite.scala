...
class LegacySessionStateSuite extends SparkFunSuite {
  ...
  override def beforeAll(): Unit = {
    ...
    activeSession = SparkSession.builder()
      ...
      .config("spark.legacy.sessionInitWithConfigDefaults", "true")
      .master("local")
      .getOrCreate()
  }
  ...
  test("fork new session and inherit RuntimeConfig overrides") {
    try {
      ...
      // inheritance - with the legacy flag set, we inherit the global default rather
      // than the parent session's override
      val forkedSession = activeSession.cloneSession()
      assert(forkedSession ne activeSession)
      assert(forkedSession.conf ne activeSession.conf)
      assert(forkedSession.conf.get(configKeyWithDefaultValue) == "global")
